...
func (s *InboundService) AddInbound(inbound *model.Inbound) error {
	...
	if exist {
		return common.NewError("The port already exists. When using the same port, please ensure that the protocol used by the port is consistent and that the ID used is not the same as the mailbox:", inbound.Port)
	}
	db := database.GetDB()
